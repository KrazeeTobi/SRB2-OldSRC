...
// -----------------+
static inline void md2_loadTexture (md2_t *model)
{
	...
	{
		grpatch = Z_Malloc(sizeof (GlidePatch_t),PU_HWRPATCHINFO,&(model->grpatch));
		memset(grpatch,0x00, sizeof (GlidePatch_t));
	}
...
void HWR_DrawMD2(gr_vissprite_t *spr)
{
	...
	if (cv_grmd2.value && (md2_models[spr->mobj->sprite].scale > 0))
	{
		...
		gpatch = md2->grpatch;
		if (!gpatch || gpatch->mipmap.downloaded == 0)
			md2_loadTexture(md2);
		if (gpatch->mipmap.grInfo.format)
