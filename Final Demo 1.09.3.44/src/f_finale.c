...
void F_StartCredits(void)
{
	...
	strcpy(credits[i].fakenames[5], "Neo Chaotikal\n");
	strcpy(credits[i].fakenames[6], "Some Guy\n");
	strcpy(credits[i].realnames[0], "Johnny Wallbank\n");
	strcpy(credits[i].realnames[1], "Art Freda\n");
	strcpy(credits[i].realnames[2], "Ben Geyer\n");
	strcpy(credits[i].realnames[3], "Marco Zafra\n");
	strcpy(credits[i].realnames[4], "Hank Brannock\n");
	strcpy(credits[i].realnames[5], "Fred Bronze\n");
	strcpy(credits[i].realnames[6], "Anonymous\n");
...
static void F_SkyScroll(void)
{
	...
	animtimer = ((finalecount*((gamestate == GS_INTRO || gamestate == GS_INTRO2) ? titlescrollspeed*4 : titlescrollspeed))/16) % 320;
	...
	{
		if (scrolled > 0)
			V_DrawScaledPatch(320 - scrolled, 0, V_WRAPX, pat);

		if (scrolled < 320)
			V_DrawScaledPatch(-scrolled, 0, V_WRAPX, pat);
	}
#endif
}
