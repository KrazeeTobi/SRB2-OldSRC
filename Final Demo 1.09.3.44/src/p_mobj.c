...
long playerstatetics[MAXPLAYERS][S_PLAY_SUPERHIT+1];
...
void P_RunCachedActions(void)
{
	...
	for(ac = actioncachehead.next; ac != NULL; ac = next)
	{
...
{
	actioncache_t *newaction = Z_Malloc(sizeof(actioncache_t), PU_LEVEL, NULL);
	newaction->mobj = mobj;
...
void P_XYMovement(mobj_t *mo)
{
	...
	if (mo->type == MT_FLINGRING && mo->z == mo->subsector->sector->floorheight
...
static void P_RingZMovement(mobj_t *mo)
{
	...
	else if (mo->z + mo->height > mo->ceilingz && !(mo->flags2 & MF2_NOCLIPHEIGHT))
	{
		mo->momz = 0;
...
static void P_ZMovement(mobj_t *mo)
{
	...
	if (mo->z + mo->height > mo->ceilingz && !(mo->flags2 & MF2_NOCLIPHEIGHT))
	{
		if (mo->momz > 0)
		{
			// hit the ceiling
		...
		if (mo->momz < 0) // falling
		{
				...
				|| tmfloorthing->type == MT_FLOORSPIKE))) // Spin Attack
			{
				...
				if (mo->health)
				{
					...
					if (!(mo->player->mfspinning && mo->player->usedown))
...
nightsdone:
	...
	if (mo->z + mo->height > mo->ceilingz)
	{
		...
		if (mo->momz > 0)
		{
			...
			if (CheckForMarioBlocks && mo->player && !(gametype == GT_CTF && !mo->player->ctfteam)) // Only let the player punch
			{
...
void P_MobjCheckWater(mobj_t *mobj)
{
		...
		&& ((mobj->eflags & MF_UNDERWATER) != wasinwater))
	{
		...
		if (mobj->momz < 0)
		{
			...
			// skipping stone!
			if (mobj->player && mobj->player->waterskip && !mobj->player->mfjumped && mobj->player->speed/2 > abs(mobj->momz>>FRACBITS)
				&& mobj->player->mfspinning)
			{
				mobj->momz = -mobj->momz/2;

				if (mobj->momz > 6*FRACUNIT)
...
void P_RecalcPrecipInSector(sector_t *sector)
{
	...
	// Why bother?
	if (!(curWeather == PRECIP_RAIN || curWeather == PRECIP_SNOW
...
#else // Not as ideal, but probably still faster than a441's old insane method
void P_RecalcPrecipInSector(sector_t *sector)
{
//	precipmobj_t *precipthing;
//	thinker_t *th;

	sector = NULL; // Advanced precip still doesn't work =(=(

	//if (!cv_advancedprecip.value)
		return;
/*
	// Why bother?
	if (!(curWeather == PRECIP_RAIN || curWeather == PRECIP_SNOW
	...
	if (!sector || sector->ceilingpic != skyflatnum)
		return;

	// scan the thinkers
	// to find precipitation in this sector
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		if (!(th->function.acp1 == (actionf_p1)P_SnowThinker
			|| th->function.acp1 == (actionf_p1)P_RainThinker
			|| th->function.acp1 == (actionf_p1)P_NullPrecipThinker))
			continue;

		precipthing = (precipmobj_t *)th;

		if (precipthing->subsector && precipthing->subsector->sector == sector)
			CalculatePrecipFloor(precipthing);
	}*/
}
#endif
...
static boolean P_ShieldLook(mobj_t *thing, powertype_t power)
{
	...
	thing->floorz = thing->target->floorz;
	thing->ceilingz = thing->target->ceilingz;
...
void P_MobjThinker(mobj_t *mobj)
{
	...
	if (mobj->subsector && mobj->subsector->sector->special == 970)
	{
		sector_t *sec2;
		...
		case MT_REDFLAG:
			{
				sector_t *sec2;
				sec2 = P_ThingOnSpecial3DFloor(mobj);
				if ((sec2 && sec2->special == 995) || (mobj->subsector->sector->special == 995))
				...
				case MT_EGGMANBOX:
					...
					if (mobj->flags & MF_AMBUSH)
					{
						...
						if (!(cv_1up.value || cv_bombshield.value || cv_fireshield.value
							...
							P_SpawnMobj(mobj->x, mobj->y, mobj->z, spawnchance[prandom%numchoices])->flags |= MF_AMBUSH;
					}
					else
...
consvar_t cv_itemrespawntime = {"respawnitemtime", "30", CV_NETVAR, CV_Natural, NULL, 0, NULL, NULL, 0, 0, NULL};
...
void P_SpawnPrecipitation(void)
{
	...
	else if (curWeather == PRECIP_STORM || curWeather == PRECIP_RAIN || curWeather == PRECIP_BLANK || curWeather == PRECIP_STORM_NORAIN)
	{
...
void P_RespawnSpecials(void)
{
	...
	if (curWeather == PRECIP_STORM || curWeather == PRECIP_RAIN || curWeather == PRECIP_STORM_NORAIN)
	{
		...
		spawnlightning = netgame ? (P_Random() < 2) : (M_Random() < 2);
		if (spawnlightning)
		{
			sector_t *ss = sectors;
		...
		if (!dedicated && playeringame[displayplayer])
		{
			...
			if (curWeather == PRECIP_STORM || curWeather == PRECIP_STORM_NORAIN)
			{
				if (spawnlightning)
				{
				...
				{
					byte mrandom = M_Random(); // This doesn't need to use P_Random().
...
void P_SpawnMapThing(mapthing_t *mthing)
{
	...
	if (mthing->type != 61 && mthing->type != 46 && mthing->type != 72
		&& mthing->type != 82 && mthing->type != 3006)
	{
		unsigned int bit;
	...
	if (mthing->type == 61 || mthing->type == 46 || mthing->type == 72) // Axis Points
	{
		// Mare it belongs to
...
void P_SpawnHoopsAndRings(mapthing_t *mthing)
{
	...
	if (mthing->type == 57) // NiGHTS hoop!
	{
		...
		hoopcenter->movedir = FixedMul((mthing->angle&255)*FRACUNIT,
			360*(FRACUNIT/256))/FRACUNIT;
		hoopcenter->movecount = AngleFixed(closestangle)/FRACUNIT;
...
mobj_t *P_SpawnMissile(mobj_t *source, mobj_t *dest, mobjtype_t type)
{
	...
	int dist;
	fixed_t z;
	const fixed_t gsf = (fixed_t)(gameskill)*2;
	fixed_t speed;
