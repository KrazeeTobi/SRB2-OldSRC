...
// $Log: s_fmod.c,v $
//
// Revision 1.1  2001/04/04 19:49:44  alam_gbc
// Start of s_fmod.dll
//-----------------------------------------------------------------------------
/// \file
...
static void relset(int which)
{
	...
	else if(which == 1 || which ==2)
	{
		...
		if(!rellistener[which].active) for(i=0; i <= STATIC_SOURCES_NUM; i++)
		{
			if(!relarray[i].pos.active && relarray[i].listener == which)
			{
...
#ifdef DEBUG_TO_FILE
			logstream = CreateFile ("s_fmod.log", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
