...
int CList::remove(CItem *item)
{
    ...
    //dbgPrintf(DEFCOL, "The List beforehand\n");
    //CList::show();
    ...
    if (position)
    {
        ...
        //dbgPrintf(DEFCOL, "The List afterward\n");
        //CList::show();
...
**
*/
CPlayerItem::CPlayerItem(const char *ip,const char *port,const char *nickname) : CInetAddr(ip, port)
{
...
**
*/
int CServerList::insert(const char *ip,const char *port,const char *hostname,const char *version, ServerType type)
{
    CServerItem *server;
...
**
*/
int CServerList::remove(const char *ip,const char *port,const char *hostname,const char *version, ServerType type)
{
    //TODO
    ...
    {
        if (strcmp(position->ip, ip) == 0 /*&& strcmp(position->port, port) == 0*/
            && strcmp(position->version, version) == 0 && (position->type == type))
        {
            //dbgPrintf(DEFCOL, "\nMatch:\n");
            //position->print();
            //dbgPrintf(DEFCOL, "\n");
