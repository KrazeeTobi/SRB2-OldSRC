		...
		if(mo2->flags2 & MF2_AXIS)
		{
			if(mo2->threshold == mare)
			{
				if(closestaxis == NULL)
				{
					...
					dist2 = R_PointToDist2(player->mo->x, player->mo->y, mo2->x, mo2->y)-mo2->info->radius;
				}
		...
		for(th = thinkercap.next ; th != &thinkercap ; th=th->next)
		{
			...
			if(mo2->flags2 & MF2_AXIS)
			{
				if(mo2->threshold == player->mare)
				{
					...
					{
						closestaxis = mo2;
						dist2 = R_PointToDist2(newx, newy, mo2->x, mo2->y)-mo2->info->radius;
					}
					else
					{
...
//////////////////
	...
	if(player->gliding)
	{
		...
		if(!player->jumpdown) // If not holding the jump button
		{
			...
			if(gametype == GT_MATCH || gametype == GT_CTF || maptol & TOL_ADVENTURE)
			{
				player->mfjumped = 1;
				P_SetPlayerMobjState(player->mo, S_PLAY_ATK1);
			}
	...
	if(!player->climbing && player->mo->z + player->mo->info->height >= player->mo->watertop && player->mo->z <= player->mo->watertop && player->speed > player->runspeed && leveltime % (TICRATE/7) == 0 && player->mo->momz == 0)
		S_StartSound(P_SpawnMobj(player->mo->x, player->mo->y, player->mo->watertop, MT_SPLISH), sfx_wslap);
...
///////////////////////////
	...
	if(player->mo->eflags & MF_UNDERWATER && !(player->powers[pw_watershield]))
	{
		...
		if(player->powers[pw_tailsfly] && (leveltime & 1))
		{
	...
	if(player->charability == 1)
	{
		...
		if(player->powers[pw_tailsfly])
		{
			...
			{
				if(player->mo->momz > -5*FRACUNIT/NEWTICRATERATIO)
					player->mo->momz -= FRACUNIT/NEWTICRATERATIO;
			}
		...
		else
		{
			...
			if((leveltime % 10 == 0) && player->mo->state >= &states[S_PLAY_SPC1]
		...
		else if((gametype != GT_CTF) || (!player->gotflag))
		{
			switch(player->charability)
			{
				...
				case 1:
					...
					else if(!player->thokked && !(player->powers[pw_tailsfly]) && (player->mfjumped))
					{
				...
				case 3:
					...
					else if(player->mfjumped && (!player->thokked || maptol & TOL_ADVENTURE))
					{
						player->gliding = 1;
						player->thokked = true;
						player->glidetime = 0;
	...
	if(!(cmd->buttons & BT_JUMP))// If not pressing the jump button
	{
		...
		if(player->charability == 5 && player->secondjump == 1)
			player->secondjump = 2;
	}

	if(player->charability == 5 && player->secondjump == 1 && (cmd->buttons & BT_JUMP))
	{
		player->mo->momz = 0;
		player->mfspinning = 0;
	}
	...
	if(cmd->buttons & BT_USE && (player->mfjumped || player->powers[pw_tailsfly]))
	{
		if(player->skin == 0 && player->powers[pw_super] && player->speed > 5 && player->mo->momz <= 0)
		{
			if(player->mo->state >= &states[S_PLAY_ATK1]
				&& player->mo->state <= &states[S_PLAY_ATK4])
				P_SetMobjState(player->mo, S_PLAY_SUPERWALK1);
...
#define EXPLOSION player->powers[pw_explosionring]
	...
	if((player->charflags & SF_SPINALLOWED) && ((player->mfspinning) || (player->mfjumped) || (player->powers[pw_tailsfly])
...
static inline void P_DoZoomTube(player_t* player)
{
	int sequence;
	fixed_t speed = player->speed;
	thinker_t* th;
	mobj_t* mo2;
	...
	// his current coordinates, or closer?
	if(dist <= P_AproxDistance(P_AproxDistance(player->mo->target->x - player->mo->x + player->mo->momx, player->mo->target->y - player->mo->y + player->mo->momy), player->mo->target->z - player->mo->z + player->mo->momz))
	{
		...
		for(th = thinkercap.next; th != &thinkercap; th = th->next)
		{
			...
			if(mo2->threshold == sequence && mo2->health == player->mo->target->health + 1)
			{
				waypoint = mo2;
				break;
			}
