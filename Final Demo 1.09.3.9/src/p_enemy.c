...
void A_Look(mobj_t* actor)
{
	if(!P_LookForPlayers(actor, var1))
		return;
...
void A_MixUp(mobj_t* actor)
{
	...
	else if(numplayers == 2) // Special case -- simple swap
	{
		...
		P_SetThingPosition(players[one].mo);
		P_SetThingPosition(players[two].mo);

		P_TryMove(players[one].mo, players[one].mo->x, players[one].mo->y, true);
		P_TryMove(players[two].mo, players[two].mo->x, players[two].mo->y, true);
	}
	else
	{
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		if(teleported[i])
		{
				...
				&& players[i].mo && players[i].mo->health > 0 && !players[i].exiting)
			{
				...
				P_TryMove(players[i].mo, players[i].mo->x, players[i].mo->y, true);
			}
		}
	}
