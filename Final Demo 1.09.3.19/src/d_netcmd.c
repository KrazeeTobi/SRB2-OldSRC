...
/** Sets a player's name or doesn't.
  ...
  */
static void SetPlayerName(int playernum, char *newname, boolean formatonly)
{
	char *p;
	char *tmpname = NULL;
	...
	do
	{
		strncpy(buf, newname, MAXPLAYERNAME-1);
		...
		for (i = 0; i < MAXPLAYERS; i++)
			if (playeringame[i] && !strcasecmp(tmpname, player_names[i]))
			{
				if (formatonly)
				{
					strncpy(buf, tmpname, MAXPLAYERNAME-1);
					CV_StealthSet(&cv_playername, buf);
					return;
				}
...
static void SendNameAndColor(void)
{
	...
	if (!netgame)
	{
		...
		SetPlayerName(0, cv_playername.zstring, false);
	...
	SetPlayerName(consoleplayer, cv_playername.zstring, true);
...
static void SendNameAndColor2(void)
{
	...
	if (!netgame || (server && secondplaya == consoleplayer))
	{
		...
		SetPlayerName(1, cv_playername2.zstring, false);
	...
	SetPlayerName(secondarydisplayplayer, cv_playername2.zstring, true);
...
static void Got_NameAndcolor(char **cp, int playernum)
{
	...
	if (strcasecmp(player_names[playernum], *cp))
		SetPlayerName(playernum, *cp, false);
	SKIPSTRING(*cp);

	// moving players cannot change colors
	if (players[playernum].mo && players[playernum].mo->health > 0 && (
		...
		players[playernum].mo->momz <= -FRACUNIT/2 ||
		players[playernum].climbing ||
		players[playernum].powers[pw_tailsfly] ||
		players[playernum].mfjumped ||
		players[playernum].mfspinning))
	{
		// if the color is the same as before it's okay, though
		// (as during everyone's SendNameAndColor when a new player joins)
		if (p->skincolor != (extrainfo&31))
		{
			SKIPSTRING(*cp);
			if (playernum == consoleplayer)
	...
	{
		if (p->skincolor != ((extrainfo&31) % MAXSKINCOLORS))
			CONS_Printf("Changed color to %s\n", Color_Names[((extrainfo&31) % MAXSKINCOLORS)]);
	}
...
static void Command_Map_f(void)
{
	...
	if (i)
	{
		...
		for (j = 0; gametype_cons_t[j].strvalue; j++)
			...
			{
				if (gametype_cons_t[j].value == 42) // Team Match
				{
					newgametype = GT_MATCH;
					CV_SetValue(&cv_teamplay, 1);
				}
				else if (gametype_cons_t[j].value == 43) // Time-Only Race
				{
					newgametype = GT_RACE;
...
static void Command_MotD_f(void)
{
	...
	strcpy(motd, COM_Argv(1));
	for (i = 2; i < j; i++)
	{
		strcat(motd, " ");
		strcat(motd, COM_Argv(i));
	}
...
void Command_ListWADS_f(void)
{
	int i = numwadfiles;
	char *tempname;
	CONS_Printf("There are %i wads loaded:\n",numwadfiles);
	for (i--; i; i--)
	{
