...
$today = getdate();
$gamelist_delay = 5;
$lastupdatetime = mktime($today['hours'],$today['minutes']-($gamelist_delay + 1),0,$today['mon'], $today['mday'], $today['year']);
$lastlisttime = mktime($today['hours'],$today['minutes']-$gamelist_delay,0,$today['mon'], $today['mday'], $today['year']);
...
class SRB2MS_BOT
{
	...
	function RSSListGames(&$irc, &$data)
	{
		...
		if ($SRB2MSRSS10)
		{
			if ((mktime($today['hours'],$today['minutes']-$gamelist_delay,0,$today['mon'], $today['mday'], $today['year'])) >= $lastlisttime)
			{
					//$irc->message(SMARTIRC_TYPE_CHANNEL, '#srb2fun', 'Running SRB2 netgames ');
					foreach ($SRB2MSRSS10->items as $item) 
					{
