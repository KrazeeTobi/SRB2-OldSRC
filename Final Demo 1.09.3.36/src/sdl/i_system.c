...
void I_OutputMsg(const char *fmt, ...)
{
	...
	len = strlen(txt);
#if (defined (_WIN32) || defined (_WIN64)) && !defined (_XBOX) && !defined(_WIN32_WCE) && !defined(HAVE_READLINE)
#ifdef DEBUGFILE
	if (debugfile != stderr)
#endif
	{
...
#ifdef HAVE_READLINE
static void SRB2Readline(char *input)
{
 	if(input == NULL)
		return;
}
...
int I_StartupSystem(void)
{
	...
	rl_callback_handler_install("SRB2: ",SRB2Readline);
#endif
#ifdef _XBOX
#ifdef __GNUC__
