...
/** Get the typeoflevel flag needed to indicate support of a gametype.
  ...
  */
static short TOLFlag(int gametype)
{
	if (!multiplayer)         return TOL_SP;
	if (gametype == GT_COOP)  return TOL_COOP;
	if (gametype == GT_RACE)  return TOL_RACE;
	if (gametype == GT_MATCH) return TOL_MATCH;
#ifdef CHAOSISNOTDEADYET
	if (gametype == GT_CHAOS) return TOL_CHAOS;
#endif
	if (gametype == GT_TAG)   return TOL_TAG;
	if (gametype == GT_CTF)   return TOL_CTF;
