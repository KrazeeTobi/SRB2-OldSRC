...
#endif
{
    ...
    if(col == DEFCOL)
    {
        if(!GetConsoleScreenBufferInfo(co,&ConsoleScreenBufferInfo))
            ConsoleScreenBufferInfo.wAttributes = FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE;
    ...
    if(col != DEFCOL) SetConsoleTextAttribute(co, ConsoleScreenBufferInfo.wAttributes);
#else
    printf("%s%s", col, str);
    fflush(stdout);
#endif
