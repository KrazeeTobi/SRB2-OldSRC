...
void HWR_WallLighting(FOutVector *wlVerts)
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		if ((dynlights->mo[j]->state->nextstate == S_NULL) || (dynlights->mo[j]->state->nextstate == S_DISS)) // Included S_DISS Tails 09-09-2002
			Surf.FlatColor.s.alpha = (byte)(((float)dynlights->mo[j]->tics/(float)dynlights->mo[j]->state->tics)*Surf.FlatColor.s.alpha);
...
void HWR_DoCoronasLighting(FOutVector *outVerts, gr_vissprite_t *spr) 
{
	...
	{ // it's an object which emits light
		...
		HWD.pfnDrawPolygon (&Surf, light, 4, PF_Modulated | PF_Additive | PF_Clip | PF_Corona | PF_ReadOnlyDepth);
	}
}
#endif
...
void HWR_DrawCoronas(void)
{
	...
	for (j = 0;j < dynlights->nb;j++)
	{
		...
		HWD.pfnDrawPolygon (&Surf, light, 4, PF_Modulated | PF_Additive | PF_Clip | PF_ReadOnlyDepth | PF_Corona);
	}
}
#endif
