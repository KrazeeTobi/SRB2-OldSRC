...

		{
			...
			if(!transfer1)
			{
				for (th = thinkercap.next; th != &thinkercap; th = th->next)
				{
					...
					if (mo2->type == MT_AXISTRANSFERTOLAST && mo2->threshold == sequence)
					{
						transfer1 = mo2;
						break;
					}
...
//					CONS_Printf("closest %d\n", transfer1->health);
				...
				{
					// Transfer1
					if(transfer1->type == MT_AXISTRANSFERTOLAST)
...
//					CONS_Printf("closest %d\n", transfer2->health);
				...
				{
					// Transfer2
					if(transfer2->type == MT_AXISTRANSFERTOLAST)
...
//					CONS_Printf("%d\n", axis->health);
	...
	if (cv_objectplace.value)
	{
		...
		if (cmd->buttons & BT_ATTACK && !player->attackdown)
		{
			...
			temp = (short)((short)(AngleFixed(player->mo->angle)/FRACUNIT)+90);
			if(temp > 359)
				temp %= 360;
