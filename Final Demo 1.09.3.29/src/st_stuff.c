...
static void ST_drawCTFHUD(void)
{
		...
		|| (stplyr->ctfteam == 2 && whichflag & MF_BLUEFLAG)))
	{
		...
		V_DrawScaledPatch(x, STRINGY(152), 0, nonicon);
	}
	...
	if (stplyr->ctfteam == 1)
	{
		if (cv_splitscreen.value)
			...
			V_DrawString(256, STRINGY(190), 0, "RED TEAM");
	}
	else if (stplyr->ctfteam == 2)
	{
		if (cv_splitscreen.value)
			...
			V_DrawString(248, STRINGY(190), 0, "BLUE TEAM");
	}
	else
	{
...
static void ST_overlayDrawer(void)
{
	...
	if ((gametype == GT_COOP || gametype == GT_RACE))
	{
		...
		if (stplyr->powers[pw_super] && stplyr->skin == 0) // SONIC ONLY
	...
	if (netgame && gametype == GT_RACE && stplyr->lives <= 0 && displayplayer == consoleplayer)
