...
void P_DoPlayerExit(player_t* player)
{
	...
	if(netgame && (gametype == GT_COOP || gametype == GT_RACE) && !circuitmap)
...
//////////////////
	...
	if(player->gliding)
	{
		...
		if((player->mo->eflags & MF_UNDERWATER))
			P_InstaThrust(player->mo, player->mo->angle-leeway, (((player->actionspd<<FRACBITS)/2) + player->glidetime*500)/NEWTICRATERATIO);
		else
			P_InstaThrust(player->mo, player->mo->angle-leeway, ((player->actionspd<<FRACBITS) + player->glidetime*1000)/NEWTICRATERATIO);
	...
	if(!player->climbing && player->mo->z + player->mo->info->height >= player->mo->watertop && player->mo->z <= player->mo->watertop && (player->speed > player->runspeed || player->mfstartdash) && leveltime % (TICRATE/7) == 0 && player->mo->momz == 0)
		S_StartSound(P_SpawnMobj(player->mo->x, player->mo->y, player->mo->watertop, MT_SPLISH), sfx_wslap);
...
/*					player->weapondelay = 2;
	...
	if((player->charflags & SF_SPINALLOWED) && ((player->mfspinning) || (player->mfjumped && !(player->charflags & SF_NOJUMPSPIN)) || (player->powers[pw_tailsfly])
