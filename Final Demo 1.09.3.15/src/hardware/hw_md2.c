...
static inline void md2_getAnimationFrames (md2_model_t *model, int animation, int *startFrame, int *endFrame)
{
	int i, j = 0;
	size_t pos;
...
static GrTextureFormat_t PCX_Load(const char *filename, int *w, int *h, GlidePatch_t *grpatch)
{
	PcxHeader          header;
	byte               palette[768];
	const byte         *pal;
...
// -----------------+
static inline int md2_loadTexture (const char *filename)
{
...
#else
				md2->texture = md2_loadTexture( md2->filename );
#endif
