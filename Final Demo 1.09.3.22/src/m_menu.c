...
//  A 160x100 image of the level as entry MAPxxP
	...
	if (lumpnum != -1)
		PictureOfLevel = W_CachePatchName(va("MAP%02dP",cv_nextmap.value), PU_CACHE);
	else
		PictureOfLevel = W_CachePatchName("BLANKLVL", PU_CACHE);
	
	V_DrawSmallScaledPatch((BASEVIDWIDTH*3/4)-(PictureOfLevel->width/4), (BASEVIDHEIGHT*3/4)-(PictureOfLevel->height/4), 0, PictureOfLevel, colormaps);
}
