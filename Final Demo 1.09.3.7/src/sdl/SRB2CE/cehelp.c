    ...
    va_list *Arguments)
{
	...
	if(lpSourceW)
		HR = FormatMessageW(dwFlags,lpSourceW,dwMessageId,dwLanguageId,lpBufferW,nSize,Arguments);
	else
		HR = FormatMessageW(dwFlags,lpSource,dwMessageId,dwLanguageId,lpBufferW,nSize,Arguments);

	WSTRtoSTR(lpBufferW,lpBuffer,nSize);
