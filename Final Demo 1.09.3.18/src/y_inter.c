...
void Y_StartIntermission(void)
{
	...
	switch(inttype)
	{
		case int_coop: // coop or single player, normal level
		{
			...
			if((!modifiedgame || savemoddata) && !multiplayer && timeattacking)
			{
...
static void Y_FollowIntermission(void)
{
	...
	{
		char nameofdemo[256];
		sprintf(nameofdemo, "%s/replay/%s/%s-%02d.lmp", srb2home, timeattackfolder, G_BuildMapName(gamemap), cv_chooseskin.value-1);
		...
		if(FIL_FileExists(va("%s/temp.lmp",srb2home)))
		{
			if(FIL_FileExists(nameofdemo))
				remove(nameofdemo);

			rename(va("%s/temp.lmp",srb2home), nameofdemo);
		}
