...
static void M_DrawStats(void)
{
	...
	if (displaytimeattack)
	{
		if (besttime/(3600*TICRATE) < 10)
			sprintf(hours, "0%i", (int)(besttime/(3600*TICRATE)));
		else
			sprintf(hours, "%i", (int)(besttime/(3600*TICRATE)));
...
static void Nextmap_OnChange(void)
{
	...
	if (!FIL_FileExists(va("%s/replay/%s/%s-%02d.lmp", srb2home, timeattackfolder, G_BuildMapName(cv_nextmap.value), cv_chooseskin.value-1)))
...
void M_DrawTimeAttackMenu(void)
{
	...
	if (W_CheckNumForName(description[cv_chooseskin.value-1].picname) != -1)
		V_DrawSmallScaledPatch(224, 16, 0, W_CachePatchName(description[cv_chooseskin.value-1].picname, PU_CACHE), colormaps);

//  A 160x100 image of the level as entry MAPxxP
	lumpnum = W_CheckNumForName(va("MAP%02dP",cv_nextmap.value));

	if (lumpnum != -1)
		PictureOfLevel = (patch_t*)W_CacheLumpNum(lumpnum, PU_CACHE);
	else
		PictureOfLevel = W_CachePatchName("BLANKLVL", PU_CACHE);
	
	V_DrawSmallScaledPatch(208, 128, 0, PictureOfLevel, colormaps);
...
static void M_ReplayTimeAttack(int choice)
{
	choice = 0;
	M_ClearMenus(true);
	G_DoPlayDemo(va("%s/replay/%s/%s-%02d", srb2home, timeattackfolder, G_BuildMapName(cv_nextmap.value), cv_chooseskin.value-1));
	timeattacking = true;
}
...
static void M_DrawLevelSelectMenu(void)
{
	...
	//  A 160x100 image of the level as entry MAPxxP
		...
		V_DrawSmallScaledPatch(200, 110, 0, PictureOfLevel, colormaps);
	}
}
