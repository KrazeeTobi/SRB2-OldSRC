...
void R_RenderThickSideRange(drawseg_t* ds, int x1, int x2, ffloor_t* pfloor)
{
    ...
    if(frontsector->numlights)
    {
      ...
      for(i = p = 0; i < dc_numlights; i++)
      {
        ...
        if(light->height > *pfloor->topheight)
          if(i+1 < dc_numlights && frontsector->lightlist[i+1].height > *pfloor->topheight)
            continue;
          ...
          for(i = 0; i < dc_numlights; i++)
          {
            ...
            else if(rlight->flags & FF_CUTEXTRA && pfloor->flags & FF_EXTRA)
            {
              if(rlight->flags & FF_EXTRA)
              {
                ...
                if((rlight->flags & (FF_TRANSLUCENT|FF_FOG)) == (pfloor->flags & (FF_TRANSLUCENT|FF_FOG)))
                  solid = 1;
              }
              else
                solid = 1;
