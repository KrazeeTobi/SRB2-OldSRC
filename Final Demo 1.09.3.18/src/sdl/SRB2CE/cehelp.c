...
{
	return MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,lpMultiByteStr,-1,lpWideCharStr,cchWideChar);
}

static inline int WSTRtoSTR(
	LPCWSTR lpWideCharStr,
	LPSTR lpMultiByteStr,
	int cbMultiByte
	)
{
	return WideCharToMultiByte(CP_ACP,MB_PRECOMPOSED,lpWideCharStr,-1,lpMultiByteStr,cbMultiByte,NULL,NULL);
}
...
DWORD WINAPI FormatMessageA(
    ...
    va_list *Arguments)
{
	LPWSTR lpBufferW = alloca(sizeof(wchar_t)*nSize);
	...
	{
		const int sLen = STRtoWSTR(lpSource, NULL, 0);
		lpSourceW = alloca(sizeof(wchar_t)*sLen);

		if(lpSourceW)
			STRtoWSTR(lpSource, lpSourceW, sLen);
		else
		{
			SetLastError(ERROR_NOT_ENOUGH_MEMORY);
			return 0;
	...
	if(lpSourceW)
		FormatMessageW(dwFlags,lpSourceW,dwMessageId,dwLanguageId,lpBufferW,nSize,Arguments);
	else
		FormatMessageW(dwFlags,lpSource,dwMessageId,dwLanguageId,lpBufferW,nSize,Arguments);

	return WSTRtoSTR(lpBufferW,lpBuffer,nSize);
}

BOOL WINAPI DeleteFileA(
    LPCSTR lpFileName)
{
	const int sLen = STRtoWSTR(lpFileName, NULL, 0);
...
BOOL WINAPI CreateDirectoryA(
    ...
    LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
	const int sLen = STRtoWSTR(lpPathName, NULL, 0);
...
int WINAPI MessageBoxA(
    ...
    UINT uType)
{
	const int sLen1 = STRtoWSTR(lpText, NULL, 0);
	LPWSTR lpTextW = alloca(sizeof(wchar_t)*sLen1);

	const int sLen2 = STRtoWSTR(lpCaption, NULL, 0);
...
{
	const int sLen = STRtoWSTR(lpOutputString, NULL, 0);
	LPWSTR lpOutputStringW = alloca(sizeof(wchar_t)*sLen);

	if(lpOutputStringW)
		STRtoWSTR(lpOutputString, lpOutputStringW, sLen);
	else
	{
		SetLastError(ERROR_NOT_ENOUGH_MEMORY);
		return;
