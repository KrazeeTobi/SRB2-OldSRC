...
static CV_PossibleValue_t granisotropicmode_cons_t[] = {{1, "MIN"}, {999999999, "MAX"}, {0, NULL}};
...
*/
	...
	if (gr_backsector)
	{
		...
		if (gr_midtexture)
		{
			...
			// set top/bottom coords
			wallVerts[2].y = wallVerts[3].y = FIXED_TO_FLOAT(h - (heightsecd ? gr_sidedef->rowoffset : 0));
			wallVerts[0].y = wallVerts[1].y = FIXED_TO_FLOAT(l - (heightsecd ? gr_sidedef->rowoffset : 0));
...
//	HWR_ResetLights();
	...
	HWD.pfnGClipRect (0,0,vid.width,vid.height, 0.99f);
}
