...
void R_AddSkins(int wadnum)
{
	...
	while((lumpnum = W_CheckForSkinMarkerInPwad(wadnum, lastlump)) != -1)
	{
		...
		while(token)
		{
			...
			else if(!strcmp(token, "endcolor")) // Not necessary, but if desired, must be defined after startcolor.
			{
