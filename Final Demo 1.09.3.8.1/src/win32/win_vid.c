...
void I_ShutdownGraphics(void)
{
	...
	{
		// swap des deux lignes comme ça on close
...
void I_FinishUpdate(void)
{
...
#endif
	{
		...
		if(LockScreen())
		{
			...
			/// too main memory on recent hardware, and software blit of directX may be
...
static void VID_Init(void)
{
	...
	if(rendermode != render_soft)
	{
		...
		if(drvname && Init3DDriver(drvname))
		{
			...
			if(hwdversion != VERSION && rendermode == render_glide)
				CONS_Printf("WARNING: This r_glide version is not supported, use it at your own risk.\n");
