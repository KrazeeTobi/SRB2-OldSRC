...
static void FIL_ForceExtension(char* path, const char* extension)
{
	char* src;
...
boolean M_SavePNG(const char* filename, void* data, int width, int height, byte* palette)
{
	...
	png_bytep png_buf;
	int num_pass = 1, pass, y, pitch = width;
	...
	if(palette)
	{
		png_colorp png_palette = (png_colorp)palette;
		...
		png_set_PLTE(png_ptr, png_info_ptr, png_palette, 256);
	}
	else
	{
		pitch = width*3;
...
#define SRB2PNGTXT 5
		...
		"Unknown";
#endif
		memset(png_text,0x00,sizeof(png_text));

		for(y = 0; y < SRB2PNGTXT; y++)
			png_text[y].key  = keytxt[y];

		png_text[0].text = titletxt;
		png_text[1].text = authortxt;
		png_text[2].text = desctxt;
		png_text[3].text = playertxt;
		png_text[4].text = interfacetxt;

		png_set_text(png_ptr, png_info_ptr, png_text, SRB2PNGTXT);
#undef SRB2PNGTXT
#endif
	...
	{
		png_buf = (png_bytep)data;
		for (y = 0; y < height; y++)
		{
			png_write_row(png_ptr, png_buf);
			png_buf += pitch;
		}
	}
...
#ifdef HAVE_PNG
	freename[10] = 'n'; freename[9] = 'p'; freename[11] = 'g';
#endif
