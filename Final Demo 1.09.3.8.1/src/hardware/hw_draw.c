...
//  0--1
	...
	v[0].sow = v[3].sow = (float)((x & flatflag)/flatsize);
	v[2].sow = v[1].sow = (float)(v[0].sow + w/flatsize);
	v[0].tow = v[1].tow = (float)((y & flatflag)/flatsize);
	v[2].tow = v[3].tow = (float)(v[0].tow + h/flatsize);
...
// --------------------------------------------------------------------------
static inline boolean saveTGA(const char* file_name, const void* buffer, int width, int height)
{
	int fd;
	TGAHeader tga_hdr;
...
#ifdef HAVE_PNG
	i = M_SavePNG(lbmname, bufw, vid.width, vid.height, NULL);
#else
	i = saveTGA(lbmname, bufw, vid.width, vid.height);
#endif
	free(bufw);
