...
#ifdef USE_RWOPS
static SDL_RWops* Smidi[2] = { NULL, NULL };
#else
...
#ifdef HAVE_MIXER
static boolean LoadSong(void* data, int lumplength, size_t select)
{
#ifdef USE_RWOPS
	SDL_RWops *musicRW = Smidi[select];
	if(musicRW->hidden.mem.base == data)
	...
	musicRW = SDL_RWFromConstMem(data,lumplength);
	...
	Smidi[select] = musicRW;
	music[select] = Mix_LoadMUS_RW(musicRW);
#else
	FILE *midfile;
	const char *tempname = va("%s/%s",MIDI_PATH,fmidi[select]);
	...
	music[select] = Mix_LoadMUS(tempname);
#endif
	return true;
}
#endif
