	...
	fixed_t wallfrac, int flags)
{
	...
	if(W_CheckNumForName(patchname) != -1) splat = R_AllocWallSplat();
	if(!splat)
		return;
	...
	int flags)
{
	...
	if(W_CheckNumForName(picname) != -1) splat = R_AllocFloorSplat();
	if(!splat)
		return;
...
{
	// rasterizing
	int miny = vid.height + 1, maxy = 0, x, y, x1, y1, x2, y2, tx, ty, tdx, tdy;
	byte* pDest;
