...
static void M_DrawConnectMenu( void )
{
	...
	for( i=0, j=0;i<serverlistcount && i+FIRSTSERVERLINE<(sizeof(ConnectMenu)/sizeof(menuitem_t));i++ )
	{
		// Don't display games from other versions!
		if(serverlist[i].info.version != VERSION
			|| serverlist[i].info.subversion != SUBVERSION)
			continue;

		strncpy(servername, serverlist[j].info.servername, 20);
		V_DrawString (currentMenu->x,currentMenu->y+(FIRSTSERVERLINE+i)*STRINGHEIGHT,0,servername);
		p = va("%d", serverlist[j].info.time);
		V_DrawString (currentMenu->x+200-V_StringWidth(p),currentMenu->y+(FIRSTSERVERLINE+i)*STRINGHEIGHT,0,p);

		switch(serverlist[j].info.gametype)
		{
			case GT_COOP:
				cgametype = 'C';
				break;
			...
			default:
				cgametype = 'U';
				CONS_Printf("M_DrawConnectMenu: Unknown gametype %d\n", serverlist[j].info.gametype);
				break;
		}

		p = va("%d/%d  %c", serverlist[j].info.numberofplayer,
		                     serverlist[j].info.maxplayer,
...
{43, "Time-Only Race"}, {GT_TAG, "Tag"}, {GT_CTF, "CTF"},
#ifdef CHAOSISNOTDEADYET
 {GT_CHAOS, "Chaos"},
#endif
 {0, NULL}};
...
static boolean M_PatchLevelNameTable(boolean levelselect)
{
	size_t i;
	int j;
	int currentmap;
	...
	for(j = 0; j < LEVELARRAYSIZE-2; j++)
	{
		...
		if(mapheaderinfo[currentmap].lvlttl[0] && !mapheaderinfo[currentmap].hideinmenu && (!levelselect || mapheaderinfo[currentmap].levelselect))
		{
...
static void M_DrawStats(void)
{
	...
	V_DrawString(64, 32, 0, va("x %i/%i", found, numemblems));
	...
	if(displaytimeattack)
	{
		if(besttime/(60*TICRATE)%60 < 10)
			sprintf(minutes, "0%i", (int)(besttime/(60*TICRATE)%60));
		else
			sprintf(minutes, "%i", (int)(besttime/(60*TICRATE)%60));

		if(((besttime/TICRATE) % 60) < 10)
			sprintf(seconds, "0%i", (int)((besttime/TICRATE) % 60));
		else
			sprintf(seconds, "%i", (int)((besttime/TICRATE) % 60));

		if((besttime % TICRATE) < 10)
			sprintf(hours, "0%i", (int)(besttime % TICRATE));
		else
			sprintf(hours, "%i", (int)(besttime % TICRATE));

		V_DrawCenteredString(224, 36, 0, "Best Time Attack:");
		V_DrawCenteredString(224, 48, 0, va("%s:%s:%s", minutes, seconds, hours));
	}
...
static void M_DrawCustomChecklist(void)
{
	...
	for(i=0; i<15; i++)
	{
		...
		if(checklist[i].unlocked)
			V_DrawString(308, 8+(24*numcustom), V_WHITEMAP, "Y");
		else
			V_DrawString(308, 8+(24*numcustom), V_WHITEMAP, "N");
