	...
	switch (netbuffer->packettype)
	{
		...
		case PT_SERVERTICS:
			fprintf(debugfile, "    firsttic %u ply %d tics %d ntxtcmd %d\n    ",
				(unsigned int)ExpandTics(netbuffer->u.serverpak.starttic), netbuffer->u.serverpak.numplayers,
			...
			fprintfstring((char *)&netbuffer->u.serverpak.cmds[netbuffer->u.serverpak.numplayers*netbuffer->u.serverpak.numtics],(byte)(
		...
		case PT_TEXTCMD2:
			...
			break;
		case PT_SERVERCFG:
			fprintf(debugfile, "    playermask %x players %d clientnode %d serverplayer %d "
				"gametic %ld gamestate %d gametype %d modifiedgame %d\n",
				(unsigned int)netbuffer->u.servercfg.playerdetected,
				netbuffer->u.servercfg.totalplayernum, netbuffer->u.servercfg.clientnode,
