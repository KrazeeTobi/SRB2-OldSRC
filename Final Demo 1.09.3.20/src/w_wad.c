...
int W_LoadWadFile(char *filename)
{
	...
	grPatch = Z_Malloc(length, PU_HWRPATCHINFO, &(wadfile->hwrcache)); // never freed
...
void W_VerifyFileMD5(int wadfilenum, const char *matchmd5)
{
	...
	if (memcmp(matchmd5, wadfiles[wadfilenum]->md5sum, 16))
		...
		I_Error(
#endif
			"File is corrupt or has been modified: %s (\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X\\x%02X)\n",
			wadfiles[wadfilenum]->filename,
			wadfiles[wadfilenum]->md5sum[0],
