...
static void ST_overlayDrawer(void)
{
	...
	if(((stplyr == &players[consoleplayer] && !cv_chasecam.value)
		|| (stplyr == &players[secondarydisplayplayer] && !cv_chasecam2.value))
		&& stplyr->powers[pw_flashing] > 0 && !(stplyr->powers[pw_flashing] & 1))
		return;
	...
	if((gametype == GT_COOP || gametype == GT_RACE))
	{
		...
		else
		{
			...
			if(!stplyr->skincolor) // 'default' color
				...
				colormap = (byte*)translationtables[stplyr->skin] - 256 + ((stplyr->powers[pw_super] ? 15 : stplyr->skincolor)<<8);
