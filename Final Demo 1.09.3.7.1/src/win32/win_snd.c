...
#define FMUSIC_LoadSongMemory(name,length) FMUSIC_LoadSongEx(name,0,length,FSOUND_LOADMEMORY,NULL,0)
#else
...
#ifdef FMODSOUND
	if(!nofmod && FSOUND_GetError() != FMOD_ERR_UNINITIALIZED)
	{
...
boolean I_StartDigSong(const char* musicname, int looping)
{
	...
	mod = FMUSIC_LoadSongMemory(data,lumplength);
...
//		}
	...
	{
		fmus = FSOUND_Stream_OpenFile(data, FSOUND_LOADMEMORY|(looping?FSOUND_LOOP_NORMAL:0), lumplength);
		if(fmus == NULL)
		{
	...
	if(fmus)
	{
		...
		*/
		if(mod)
		{
			...
			{
				if(!FMUSIC_SetMasterVolume(mod, fmodvol) && devparm)
					CONS_Printf("FMOD(Start,FMUSIC_SetMasterolume): %s\n",
					            FMOD_ErrorString(FSOUND_GetError()));
			}
			else if(devparm)
				CONS_Printf("FMOD(Start,FMUSIC_GetType): %s\n", FMOD_ErrorString(FSOUND_GetError()));
		}
		if(fmus)
		{
			if(!FSOUND_SetVolume(fsoundchannel, fmodvol) && devparm)
				CONS_Printf("FMOD(Start,FSOUND_SetVolume): %s\n",
					FMOD_ErrorString(FSOUND_GetError()));
		}
		return true;
	}
...
int I_RegisterSong(void* data, int len)
{
	...
	if(!pMidiFileData)
	{
		CONS_Printf("Not a valid MIDI file\n");
		return 0;
	}
#ifdef DEBUGMIDISTREAM
	else
