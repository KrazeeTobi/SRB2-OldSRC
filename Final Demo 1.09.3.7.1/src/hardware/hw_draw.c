...
// --------------------------------------------------------------------------
static inline void saveTGA(const char* file_name, const void* buffer, int width, int height)
{
	int fd;
	TGAHeader tga_hdr;
...
#ifdef HAVE_PNG
	M_SavePNG(lbmname, bufw, vid.width, vid.height, NULL);
#else
	saveTGA(lbmname, bufw, vid.width, vid.height);
#endif
	free(bufw);
	return true;
}
