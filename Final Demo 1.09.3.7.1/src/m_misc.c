...
boolean M_SavePNG(const char* filename, void* data, int width, int height, byte* palette)
{
	...
	{
		png_color_8 sig_bit = {8,8,8,0,0}; png_set_bgr(png_ptr);
		png_set_IHDR(png_ptr, png_info_ptr, width, height, 8, PNG_COLOR_TYPE_RGB,
       PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
		png_set_sBIT(png_ptr, png_info_ptr, &sig_bit); width *= 3;
	}

#if 0 //Write info text
	{
		png_text text_ptr[3];
 ...
 */
	...
	for (pass = 0; pass < num_pass; pass++)
	{
		...
		for (y = 0; y < height; y++)
		{
			png_write_rows(png_ptr, &png_buf, 1);
			png_buf += width;
		}
	}

#if defined(PNG_WRITE_UNKNOWN_CHUNKS_SUPPORTED)
   //png_free_data(png_ptr, png_info_ptr, PNG_FREE_UNKN, -1);
#endif
