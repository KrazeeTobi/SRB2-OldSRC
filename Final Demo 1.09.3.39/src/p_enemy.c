...
void A_Look(mobj_t *actor)
{
	if (!P_LookForPlayers(actor, var1, false))
		return;
	...
	else if (var2 == 1) // Only go into seestate
		P_SetMobjState(actor, actor->info->seestate);
	else if (var2 == 2) // Only play seesound
		A_PlaySeeSound(actor);
}
...
void A_FireShot(mobj_t *actor)
{
	...
	z = actor->z + 48*FRACUNIT + var2*FRACUNIT;

	P_SpawnXYZMissile(actor, actor->target, var1, x, y, z);
...
void A_SuperFireShot(mobj_t *actor)
{
	...
	mo = P_SpawnXYZMissile(actor, actor->target, var1, x, y, z);
...
void A_BunnyHop(mobj_t *actor)
{
	...
	{
		actor->momz = var1*FRACUNIT; // make it hop!
		actor->angle += P_Random()*FINEANGLES;
		P_InstaThrust(actor, actor->angle, var2*FRACUNIT); // Launch the hopping action! PHOOM!!
	}
}
...
void A_BubbleRise(mobj_t *actor)
{
	...
	{
		actor->momz += var2; // make bubbles rise!
...
/** Checks if a bubble should be drawn or not.
  * Bubbles are not drawn above water.
  *
...
void A_DropMine(mobj_t *actor)
{
	...
	P_SpawnMobj(actor->x, actor->y, actor->z - 12*FRACUNIT + var1, actor->info->raisestate)
...
void A_CapeChase(mobj_t *actor)
{
	...
	actor->x = chaser->x;
	actor->y = chaser->y;
	actor->z = chaser->z + chaser->height/2;
...
//
// var1 = duration before disappearing
//
void A_SnowBall(mobj_t *actor)
{
	P_InstaThrust(actor, actor->angle, actor->info->speed);
	if (!actor->fuse)
		actor->fuse = var1*TICRATE;
}
...
void A_CrawlaCommanderThink(mobj_t *actor)
{
	...
	if (!actor->fuse && actor->health == 1 && actor->z <= actor->floorz)
	{
		if (dist < 256*FRACUNIT)
		{
			actor->momz = var2;
			actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x, actor->target->y);
			P_InstaThrust(actor, actor->angle, var2/8);
			// pogo on player
		}
		else
		{
			...
			P_InstaThrust(actor, actor->angle, var2/3*2);
			actor->momz = var2; // Bounce up in air
		}
	}
...
void A_RingExplode(mobj_t *actor)
{
	...
	for (i = 0; i < 32; i++)
	{
		...
		mo = P_SpawnMobj(actor->x, actor->y, actor->z, var1);
...
// Mix up all of the player positions.
void A_MixUp(mobj_t *actor)
{
...
void A_Boss2PogoSFX(mobj_t *actor)
{
	...
	else
	{
		byte prandom = P_Random();
		actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x, actor->target->y) + (P_Random() & 1 ? -prandom : +prandom);
		P_InstaThrust(actor, actor->angle, FixedMul(actor->info->speed,(var2)));
	}
	if (actor->info->activesound) S_StartSound(actor, actor->info->activesound);
	actor->momz = var1; // Bounce up in air
	actor->reactiontime = 1;
}
...
void A_TurretFire(mobj_t *actor)
{
	...
	while (P_SupermanLook4Players(actor) && count < MAXPLAYERS)
	{
		if (P_AproxDistance(actor->x - actor->target->x, actor->y - actor->target->y) < dist)
		{
			...
			actor->eflags &= 65535;
			actor->eflags += (var1 << 16); // Upper 16 bits contains mobj #
			break;
		}
...
void A_TurretStop(mobj_t *actor)
{
	...
	if (actor->target && actor->info->activesound && !var1)
		S_StartSound(actor, actor->info->activesound);
}
...
/** Runs a linedef executor.
  ...
  *              is used to set the calling sector. The tag used is var1, or,
  *              if that is zero, the mobj's state number (beginning from 0)
  *              plus 1000.
  * \sa A_LinedefExecute
  * \author Graue <graue@oceanbase.org>
  */
...
void A_SmokeTrailer(mobj_t *actor)
{
	...
	th = P_SpawnMobj(actor->x-actor->momx, actor->y-actor->momy, actor->z, var1);
...
//
// Spawns an object at an absolute position
//
// var1 >> 16 = x
// var1 & 65535 = y
// var2 >> 16 = z
...
void A_SpawnObjectAbsolute(mobj_t *actor)
{
	...
	x = (signed short)(var1>>16);
	y = (signed short)(var1&65535);
	z = (signed short)(var2>>16);
	type = (mobjtype_t)(var2&65535);
...
//
// Spawns an object relative to the location of the actor
//
// var1 >> 16 = x
// var1 & 65535 = y
// var2 >> 16 = z
...
//
// Changes the angle to a random relative value between
// the min and max
//
// Set min and max to the same value to eliminate randomness
//
// var1 = min
//
// var2 = max
void A_ChangeAngleRelative(mobj_t *actor)
{
	...
	const angle_t amin = FixedAngle(var1*FRACUNIT);
	const angle_t amax = FixedAngle(var2*FRACUNIT);
	angle *= (P_Random()+1);
	angle *= (P_Random()+1);
	angle *= (P_Random()+1);
...
//
// Changes the angle to a random absolute value between
// the min and max
//
// Set min and max to the same value to eliminate randomness
//
...
void A_PlaySound(mobj_t *actor)
{
	S_StartSound(var2 ? NULL : actor, var1);
}

//
// Finds the nearest/furthest mobj of the specified
// type and sets actor->target to it.
//
// var1 = mobj type
//
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		...
		if (mo2->type == (mobjtype_t)var1)
		{
			...
			else
			{
				...
				if ((!var2 && dist1 < dist2) || (var2 && dist1 > dist2))
				{
					targetedmobj = mo2;
					dist2 = dist1;
				}
...
// A_SetTics
//
// Sets the animation tics of an object
//
// var1 = tics to set to
//
...
void A_ChangeColorRelative(mobj_t *actor)
{
	if (var1)
	{
		...
		actor->flags = (actor->flags & ~MF_TRANSLATION) | (((((actor->flags & MF_TRANSLATION) >> (MF_TRANSSHIFT-8))/256)+var2)<<MF_TRANSSHIFT);
}
...
void A_ChangeColorAbsolute(mobj_t *actor)
{
	if (var1)
	{
		...
		actor->flags = (actor->flags & ~MF_TRANSLATION) | (var2<<MF_TRANSSHIFT);
}

//
// A_MoveRelative
//
// Moves an object (wrapper for P_Thrust)
//
// var1 = angle
//
// var2 = force
...
void A_MoveRelative(mobj_t *actor)
{
	P_Thrust(actor, actor->angle+(var1*(ANG45/45)), var2*FRACUNIT);
}

// A_MoveAbsolute
//
// Moves an object (wrapper for P_InstaThrust)
//
// var1 = angle
//
// var2 = force
...
void A_MoveAbsolute(mobj_t *actor)
{
	P_InstaThrust(actor, (var1*(ANG45/45)), var2*FRACUNIT);
}

// A_SetTargetsTarget
//
// Sets your target to the object who has your target targeted. Yikes!
// If it happens to be NULL, you're just out of luck.
//
void A_SetTargetsTarget(mobj_t *actor)
{
	mobj_t *targetedmobj = NULL;
...
// A_BossJetFume
//
// Spawns jet fumes for the boss.
// To only be used when he is spawned.
//
// var1:
...
void A_BossJetFume(mobj_t* actor)
{
	...
	else if (var1 == 1) // Boss2 jet fumes
	{
...
//
// A_RandomState
// Chooses one of the two state numbers supplied randomly.
//
// var1 = state number 1
// var2 = state number 2
//
void A_RandomState(mobj_t *actor)
{
	P_SetMobjState(actor, P_Random()&1 ? var1 : var2);
}

//
// A_RandomStateRange
// Chooses a random state within the range supplied.
//
