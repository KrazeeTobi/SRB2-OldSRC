...
//
static inline fixed_t P_PartialDistance(fixed_t dx, fixed_t dy)
{
	dx >>= FRACBITS;
	dy >>= FRACBITS;
...
void P_UnsetThingPosition(mobj_t *thing)
{
	...
	{
		if (thing->subsector == NULL || thing->subsector->sector == NULL)
			return;

		// Invisible things don't need to be in subsector.
...
//#endif
		...
		// delete any nodes for sectors the Thing has vacated. Then
		// we'll put it back into touching_sectorlist. It's done this
		// way to avoid a lot of deleting/creating for nodes, when most
		// of the time you just get back what you deleted anyway.
		//
...
void P_SetThingPosition(mobj_t *thing)
{
	...
	if (!(thing->flags & MF_NOSECTOR))
	{
		...
		// Don't link a thing we've already linked.
		I_Assert(thing->sprev == NULL);
		I_Assert(thing->snext == NULL);
	...
	// Link into blockmap.
 	if (!(thing->flags & MF_NOBLOCKMAP)) // Non-inert things only.
	{
		blockx = (thing->x - bmaporgx)>>MAPBLOCKSHIFT;
		blocky = (thing->y - bmaporgy)>>MAPBLOCKSHIFT;
