...
*/
	...
	xfracstep = (ptexturewidth << 16) / pblockwidth;
	yfracstep = (ptextureheight<< 16) / pblockheight;
	if (bpp < 1 || bpp > 4)
	...
	for (block += col*bpp; ncols--; block += bpp, xfrac += xfracstep)
	{
		...
		scale_y = (pblockheight << 16) / ptextureheight;
...
void HWR_PrepLevelCache(int pnumtextures)
{
	...
	gr_textures = malloc(sizeof (GlideTexture_t) * pnumtextures);
	if (gr_textures == NULL)
		I_Error("3D can't alloc gr_textures");
	memset(gr_textures, 0, sizeof (GlideTexture_t) * pnumtextures);
}
...
void HWR_GetMappedPatch(GlidePatch_t *gpatch, const byte *colormap)
{
	...
	newmip = malloc(sizeof *newmip);
	grmip->nextcolormap = newmip;
	memset(newmip, 0, sizeof *newmip);
	...
	int blockmodulo, pic_t *pic, int bpp)
{
	...
	stepy = ((int)SHORT(pic->height)<<16)/pblockheight;
	stepx = ((int)SHORT(pic->width)<<16)/pblockwidth;
