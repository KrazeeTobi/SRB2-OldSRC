...
CServerStats::CServerStats()
{
	...
	num_http_con = 0;
	num_servers = 0;
	strcpy(motd, "Welcome to the SRB2 Master Server!<br>No Clans, advertising websites, or Role Playing Servers Allowed");
	num_add = 0;
	num_removal = 0;
	num_retrieval = 0;
	num_autoremoval = 0;
...
const char *CServerStats::getLastServers()
{
	...
	for (int i = 0; i < NUMLASTSERVERS; i++)
	{
		...
		sprintf(str, "Address: %15s   Name: %-31s   v%-7s   (%s %s)\n000043210000", 
