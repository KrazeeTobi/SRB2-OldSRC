...
static void SV_SendServerInfo(int node, tic_t servertime)
{
	...
	netbuffer->u.serverinfo.numberofplayer = (unsigned char)(SHORT(numplayers) - (dedicated ? 1 : 0));
...
static boolean SV_SendServerConfig(int node)
{
	...
	
	{
		...
		if (debugfile)
		{
			fprintf(debugfile, "ServerConfig Packet about to be sent, size of packet:%u to node:%i\n",
...
#ifdef PARANOIA
	{
		int i, realplayers = dedicated ? 1 : 0;
		for (i = 0; i < MAXPLAYERS; i++)
			if (playeringame[i])
				realplayers++;
		if (realplayers != doomcom->numplayers)
			...
			else
			{
				...
				if (!nodeingame[node])
				{
					...
					if (!SV_SendServerConfig(node))
					{
						...
						/// \todo fix this !!!
						//CONS_Printf("Internal Error 5: node %i lost\n",node);
