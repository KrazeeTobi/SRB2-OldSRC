...
//-----------------------------------------------------------------------------
//
// Copyright (C) 1998-2000 by Sonic Team Junior.
//
...
//-----------------------------------------------------------------------------
/// \file
/// \brief Windows specific part of the OpenGL v1.1 API for Sonic Robo Blast 2
///
...
{
	LPCVOID renderer;
	const UINT Wflags = SWP_NOACTIVATE|SWP_NOMOVE;
	const UINT Fflags = SWP_NOACTIVATE|SWP_NOZORDER;
...
// TODO: if not fullscreen, skip display stuff and just resize viewport stuff ...
	...
	if ( WantFullScreen )
	{
		...
		SetWindowPos( hWnd, HWND_TOPMOST, 0, 0, lvid->width, lvid->height, Fflags);
	}
	else // TODO: get right titlebar height / window border size
		SetWindowPos( hWnd, HWND_NOTOPMOST, 0, 0, lvid->width+4, lvid->height+24, Wflags);
