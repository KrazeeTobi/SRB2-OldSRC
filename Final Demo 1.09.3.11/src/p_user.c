...
void P_DoPlayerExit(player_t* player)
{
	...
	if(netgame && (gametype == GT_COOP || gametype == GT_RACE))
...
///////////////////////////
	...
	if(player->skincolor && player->mfspinning && player->speed > 15 && !player->mfjumped)
	{
		...
		if(player->auraitem > 0)
			type = player->auraitem;
		else
...
boolean PIT_NukeEnemies(mobj_t* thing)
{
	...
	if(netgame && thing->player && bombsource->player)
		CONS_Printf("%s got nuked by %s!\n", player_names[thing->player-players], player_names[bombsource->player-players]);
