...
static void P_PlayerZMovement(mobj_t* mo)
{
	...
	if(mo->subsector->sector->ffloors)
	{
		...
		for(rover = mo->subsector->sector->ffloors; rover; rover = rover->next)
		{
			...
			if((!(rover->flags & FF_SOLID || rover->flags & FF_QUICKSAND) && !(mo->player && !mo->player->nightsmode && ((mo->player->charflags & SF_RUNONWATER) || mo->player->powers[pw_super]) && (rover->flags & FF_SWIMMABLE) && !mo->player->mfspinning && mo->player->speed > 28 && /*mo->ceilingz - *rover->topheight >= mo->height && */mo->z < *rover->topheight + 30*FRACUNIT && mo->z > *rover->topheight - 30*FRACUNIT)))
				continue;
...
void P_MobjCheckWater(mobj_t* mobj)
{
		...
		&& ((mobj->eflags & MF_UNDERWATER) != wasinwater))
	{
		...
		if(mobj->momz < 0)
		{
			...
			// skipping stone!
			if(mobj->player && !mobj->player->mfjumped && mobj->player->speed/2 > abs(mobj->momz>>FRACBITS)
...
void P_RespawnSpecials(void)
{
	...
	// Rain spawning
	if(curWeather == PRECIP_STORM || curWeather == PRECIP_RAIN)
	{
		int volume;
