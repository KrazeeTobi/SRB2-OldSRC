...
static void ST_overlayDrawer(void)
{
	...
	if(gametype == GT_RACE && stplyr->lives <= 0 && displayplayer == consoleplayer)
...
//							I_FinishUpdate(); // page flip or blit buffer
	...
	if(gametype == GT_RACE)
	{
		if (leveltime > TICRATE && leveltime <= 2*TICRATE)
			ST_DrawOverlayNum(SCX(BASEVIDWIDTH/2), (int)(SCY(BASEVIDHEIGHT/2)), 3, tallnum);
		else if(leveltime > 2*TICRATE && leveltime <= 3*TICRATE)
			ST_DrawOverlayNum(SCX(BASEVIDWIDTH/2), (int)(SCY(BASEVIDHEIGHT/2)), 2, tallnum);
		else if(leveltime > 3*TICRATE && leveltime <= 4*TICRATE)
			ST_DrawOverlayNum(SCX(BASEVIDWIDTH/2), (int)(SCY(BASEVIDHEIGHT/2)), 1, tallnum);
		else if(leveltime > 4*TICRATE && leveltime <= 5*TICRATE)
			V_DrawCenteredString(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, 0, "GO!");

		if(circuitmap)
		{
			if(stplyr->exiting)
				V_DrawString(BASEVIDWIDTH-80, BASEVIDHEIGHT-32, V_WHITEMAP, "FINISHED!");
			else
				V_DrawString(BASEVIDWIDTH-80, BASEVIDHEIGHT-32, 0, va("Lap: %d/%d", stplyr->laps+1, cv_numlaps.value));
		}
	}
