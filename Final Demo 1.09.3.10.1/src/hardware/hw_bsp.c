...
static poly_t* HWR_AllocPoly (int numpts)
{
	...
	if (gr_ppfree < size)
		I_Error ("allocpoly() : no more memory %u bytes left, %u bytes needed\n\n%s\n", 
...
#else
	if (gr_ppfree < size)
		I_Error ("allocvertex() : no more memory %u bytes left, %u bytes needed\n\n%s\n", 
		          gr_ppfree, size, "You can try the param -polypoolsize 2048 (or higher if needed)");
...
static void HWR_FreePoly (poly_t* poly)
{
	...
	Z_Free(poly);
#else
	size_t size = sizeof(poly_t) + sizeof(polyvertex_t) * poly->numpts;
	memset(poly,0,size);
	//mempoly -= polysize;
#endif
}
...
static poly_t* CutOutSubsecPoly (seg_t* lseg, int count, poly_t* poly)
{
	...
	int          nump=0,ps,pe;
	polyvertex_t vs = {0,0,0},ve = {0,0,0},p1 = {0,0,0},p2 = {0,0,0};
	float        fracs=0.0f;
	...
	if (bspnum & NF_SUBSECTOR)
	{
		...
		else
		{
			...
			if (ls_count-- <= 0)
			{
				...
				V_DrawCenteredString(BASEVIDWIDTH/2, 40, V_WHITEMAP, "OPENGL MODE IS UNSUPPORTED");
				V_DrawCenteredString(BASEVIDWIDTH/2, 50, V_WHITEMAP, "AND MAY CRASH YOUR");
				V_DrawCenteredString(BASEVIDWIDTH/2, 60, V_WHITEMAP, "COMPUTER.");
				V_DrawCenteredString(BASEVIDWIDTH/2, 80, V_WHITEMAP, "USE AT OWN RISK.");
