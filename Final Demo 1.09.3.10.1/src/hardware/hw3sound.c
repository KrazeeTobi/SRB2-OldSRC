...
*/
			ambient_source.left.handle = HW3DS.pfnReloadSource(ambient_source.left.handle, sfx->length);
			//ambient_source.right.handle = HW3DS.pfnReloadSource(ambient_source.right.handle, &outphased_sfx);
			ambient_source.right.handle = HW3DS.pfnReloadSource(ambient_source.right.handle, sfx->length);
	...
	else
	{
		...
		if (origin && c_type == CT_NORMAL)
		{
			...
			source->handle = HW3DS.pfnAddSource(source_parm, sfx->length);
		}
		else
			source->handle = HW3DS.pfnAddSource(NULL, sfx->length);
...
static inline float AmbientPos(angle_t an)
{
