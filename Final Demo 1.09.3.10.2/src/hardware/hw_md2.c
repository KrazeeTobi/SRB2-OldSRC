...
static GrTextureFormat_t PNG_Load(const char *filename, int *w, int *h, int *p, GlidePatch_t *grpatch)
{
	png_structp png_ptr;
	png_infop png_info_ptr;
	png_uint_32 pitch;
	png_bytep PNG_image, *row_pointers;
	...
	pitch = png_get_rowbytes(png_ptr, png_info_ptr);

	PNG_image = Z_Malloc( pitch*height, PU_HWRCACHE, &grpatch->mipmap.grInfo.data );
...
static GrTextureFormat_t PCX_Load(const char *filename, int *w, int *h, int *p, GlidePatch_t *grpatch)
{
	PcxHeader       header;
	unsigned char*  image, palette[768];
	...
	*p = *w = pw = header.xmax - header.xmin + 1;
	*h = ph = header.ymax - header.ymin + 1;
	image = Z_Malloc( pw*ph, PU_HWRCACHE, &grpatch->mipmap.grInfo.data );
...
int md2_loadTexture (const char *filename)
{
	...
	//if (!grpatch->mipmap.downloaded && !grpatch->mipmap.grInfo.data)
	{
		int w,h, p;
#ifdef HAVE_PNG
		grpatch->mipmap.grInfo.format = PNG_Load(filename, &w, &h, &p, grpatch);
		if(grpatch->mipmap.grInfo.format == 0)
#endif
		grpatch->mipmap.grInfo.format = PCX_Load(filename, &w, &h, &p, grpatch);
		if(grpatch->mipmap.grInfo.format == 0)
			return 0;
