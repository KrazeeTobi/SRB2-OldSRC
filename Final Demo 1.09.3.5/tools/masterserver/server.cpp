...
static void sendHttpServersList(int id)
{
    ...
    {
        sendHttpLine(id, "<A NAME=\"%23s\">IP: %15s    Port: %5s    Name: %-31s    Version: %7s </A>",
...
static void sendtextServersList(int id)
{
    ...
    {
        sendHttpLine(id, "IP: %15s | Port: %5s | Name: %s | Version: %7s",
...
static void updateServerHB()
{
    ...
    //dbgPrintf(DEFCOL, "HB from %s:%s", fromIP, fromPort);
    while (p && !match)
    {
        //dbgPrintf(DEFCOL, " testing %s:%s", p->getIP(), p->getPort());
        if (!strcmp(fromIP, p->getIP()) && !strcmp(fromPort, p->getPort()))
        {
    ...
    // ok, if the HeartBeat doesn't match both IP and port, check on just IP,
    // and update the port of that ServerItem, yes Mystic, I'm talking about
    // you and your bad NAT router
    p = (CServerItem *) servers_list.getFirst(); // go back to the start of the list
    while (p && !match)
    {
        if (!strcmp(fromIP, p->getIP()))
        {
            //dbgPrintf(DEFCOL, " found %s", p->getIP());
...
{
    //this would be the part of reading the PT_SERVERINFO packet, but i'm not
    //about to backport that slopy code
    return INVALID_MSG;
}
...
static int UDPMessage(long size, char* buffer)
{
    if (size == 4)
    {
        //dbgPrintf(LIGHTGREEN, "Got an HeartBeat message\n");
        updateServerHB();
        return 0;
    }
    else if (size > 58)
