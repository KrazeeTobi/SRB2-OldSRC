...
static void P_NetArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		WRITELONG(save_p, players[i].lives);
		WRITELONG(save_p, players[i].continues);
		WRITEBYTE(save_p, players[i].timeover);
		...
		WRITELONG(save_p, players[i].bumpertime);
...
static void P_NetUnArchivePlayers(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].timeover = READBYTE(save_p);
		...
		players[i].bumpertime = READLONG(save_p);
...
static void P_NetUnArchiveWorld(void)
{
	...
	for (;;)
	{
		...
		if (i > numsectors)
			I_Error("Invalid sector number %d from server", i);
...
static void P_NetArchiveThinkers(void)
{
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		if (th->function.acp1 == (actionf_p1)P_MobjThinker)
		{
			...
			// Special case for hoops
			if (mobj->type == MT_HOOPCOLLIDE || mobj->type == MT_HOOP)
			{
				if (mobj->hnext)
					WRITEULONG(save_p, mobj->hnext->mobjnum);
				else
					WRITEULONG(save_p, 0xFFFFFFFF);

				if (mobj->hprev)
					WRITEULONG(save_p, mobj->hprev->mobjnum);
				else
					WRITEULONG(save_p, 0xFFFFFFFF);
			}
		...
		switch (tclass)
		{
			case tc_mobj:
				...
				if (mobj->type == MT_HOOPCOLLIDE || mobj->type == MT_HOOP)
				{
					...
					if (value != 0xFFFFFFFF)
						mobj->hnext = (mobj_t *)value;

					value = READULONG(save_p);
					if (value != 0xFFFFFFFF)
						mobj->hprev = (mobj_t *)value;
				}
...
static void P_RelinkPointers(void)
{
		...
		currentthinker = currentthinker->next)
	{
		if (currentthinker->function.acp1 == (actionf_p1)P_MobjThinker)
		{
			...
			if (mobj->type == MT_HOOPCOLLIDE || mobj->type == MT_HOOP)
			{
				if ((size_t)mobj->hnext != (size_t)0xFFFFFFFF)
					mobj->hnext = FindNewPosition((size_t)mobj->hnext);
				else
					mobj->hnext = NULL;

				if ((size_t)mobj->hprev != (size_t)0xFFFFFFFF)
					mobj->hprev = FindNewPosition((size_t)mobj->hprev);
				else
					mobj->hprev = NULL;

				if (!mobj->hnext)
					CONS_Printf("hnext not found on %d\n", mobj->type);

				if (!mobj->hprev)
					CONS_Printf("hprev not found on %d\n", mobj->type);
			}
...
static boolean P_UnArchiveMisc(void)
{
	...
	// Don't allow players to load Ultimate mode savegames.
	if (gameskill == sk_insane)
		I_Error("Save game corrupt.\n");
