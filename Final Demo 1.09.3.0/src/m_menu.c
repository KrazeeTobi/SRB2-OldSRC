...
char LevelSelectNames[levelselect_end][33];
static void M_LevelSelect (int choice)
{
	int i, j;
	...
	for(i=spstage_start-1; i<1035; i++)
	{
		if(!(mapheaderinfo[i].typeoflevel & TOL_SP))
			continue;

		if(mapheaderinfo[i].actnum)
		{
			if(mapheaderinfo[i].nozone)
				strcpy(LevelSelectNames[j], va("%s %d", mapheaderinfo[i].lvlttl, mapheaderinfo[i].actnum));
			else
				strcpy(LevelSelectNames[j], va("%s zone %d", mapheaderinfo[i].lvlttl, mapheaderinfo[i].actnum));
		}
		else
		{
			if(mapheaderinfo[i].nozone)
				strcpy(LevelSelectNames[j], va("%s", mapheaderinfo[i].lvlttl));
			else
				strcpy(LevelSelectNames[j], va("%s zone %d", mapheaderinfo[i].lvlttl, mapheaderinfo[i].actnum));
		}

		LevelSelectMenu[j].text = LevelSelectNames[j];

		j++;

		if(j >= levelselect_end)
			break;
	}
