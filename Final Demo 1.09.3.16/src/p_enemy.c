...
void A_MixUp(mobj_t* actor)
{
	...
	else if(numplayers == 2) // Special case -- simple swap
	{
		...
		players[one].mo->flags |= MF_NOCLIP;
		players[two].mo->flags |= MF_NOCLIP;
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		if(teleported[i])
		{
				...
				&& players[i].mo && players[i].mo->health > 0 && !players[i].exiting)
			{
				...
				sector = R_PointInSubsector(players[i].mo->x, players[i].mo->y)->sector;
				if(players[i].mo->floorz < sector->floorheight)
					players[i].mo->floorz = sector->floorheight;

				if(players[i].mo->ceilingz > sector->ceilingheight)
					players[i].mo->ceilingz = sector->ceilingheight;
			}
		}
	}
...
void A_Boss2Chase(mobj_t* actor)
{
	...
	actor->target->angle += FixedAngle(FixedMul(actor->watertop,(actor->info->spawnhealth*(FRACUNIT/4)*3)/actor->health)); // Don't use FixedAngleC?
	...
	if(leveltime % (actor->health*15/10)-1 == 0)
	{
