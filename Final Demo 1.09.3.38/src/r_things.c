...
void SetPlayerSkinByNum(int playernum, int skinnum)
{
	if (skinnum >= 0 && skinnum < numskins) // Make sure it exists!
	{
		...
		if (players[playernum].jumpfactor <= 0)
			players[playernum].jumpfactor = 1;
...
#define GETSKINATTRIB_(field) GETSKINATTRIB(#field, field)
			...
			GETSKINATTRIB_(spin)
			GETSKINATTRIB("allowsuper", super)
			GETSKINATTRIB_(superanims)
			GETSKINATTRIB_(superspin)
			GETSKINATTRIB_(supercolor)
			GETSKINATTRIB_(runonwater)
			GETSKINATTRIB_(nojumpspin)
			GETSKINATTRIB_(multiability)
			GETSKINATTRIB_(lightdash)
			GETSKINATTRIB_(homing)
			GETSKINATTRIB_(ringslinger)
			GETSKINATTRIB_(slingitem)
			GETSKINATTRIB_(thokitem)
			GETSKINATTRIB_(spinitem)
			GETSKINATTRIB_(waterskip)
			...
			else if (!stricmp(stoken, "startcolor"))
			{
				...
				// Generate the ENDCOLOR based on the
				// startcolor (startcolor+16)
			...
			// must be defined after startcolor.
			GETSKINATTRIB("endcolor", endtranscolor)

			GETSKINATTRIB_(prefcolor)
			GETSKINATTRIB("jumpheight", jumpfactor)
			else if (!strcmp(stoken, "boxindex"))
			{
				STRBUFCPY(skins[numskins].boxindex, value);
