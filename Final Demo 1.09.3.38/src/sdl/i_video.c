...
#if !defined (HAVE_IMAGE) && !(defined (DC) || defined (_WIN32_WCE) || defined (PSP))
...
{
	int vfBPP;
	SDL_Surface *VidSur = SDL_GetVideoSurface();
...
#ifdef HAVE_TERMOIS
// TERMOIS console code from Quake3: thank you!
...
static inline void Sys_ConsoleInputShutdown( void )
{
	if (consolevent)
	{
...
static inline void Sys_ConsoleInputInit( void )
{
	struct termios tc;
...
static inline const char *Sys_ConsoleInput(void)
{
...
{
	const char *buff = Sys_ConsoleInput();
	if(buff) COM_BufAddText(buff);
...
void I_StartupGraphics(void)
{
	...
	//static char SDLVIDEOMID[] = "SDL_VIDEO_CENTERED=1";
