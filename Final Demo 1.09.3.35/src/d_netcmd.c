...
/** Checks if a name (as received from another player) is okay.
  ...
  * allows space), and no other player is currently using it.
  ...
  */
static boolean IsNameGood(const char *name, int playernum)
{
	int ix;
...
/** Sets a player's name, if it is good.
  ...
  * \author Graue <graue@oceanbase.org>
  */
static void SetPlayerName(int playernum, const char *newname)
{
...
static void SendNameAndColor(void)
{
	...
	if (!netgame)
	{
		...
		CleanupPlayerName(0, cv_playername.zstring);
...
static void Got_NameAndColor(char **cp, int playernum)
{
	...
	// name
	if (strcasecmp(player_names[playernum], *cp) != 0)
	...
	if (P_PlayerMoving(playernum)
		&& strcasecmp(skins[players[playernum].skin].name, *cp) != 0)
	{
		SKIPSTRING(*cp);
		if (playernum == consoleplayer)
