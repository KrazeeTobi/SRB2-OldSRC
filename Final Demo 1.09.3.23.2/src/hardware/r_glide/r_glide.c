...
EXPORT void HWRAPI(     SetBlend ) ( FBITFIELD PolyFlags )
{
	...
	if ( Xor&PF_NoDepthTest || Xor&PF_ReadOnlyDepth)
	{
...
EXPORT void HWRAPI( SetTransform ) (FTransform *transform_parm)
{
	...
	sinx = sin( grTransform.anglex*2*M_PI/360);
	cosx = cos( grTransform.anglex*2*M_PI/360);
	siny = sin( grTransform.angley*2*M_PI/360);
	cosy = cos( grTransform.angley*2*M_PI/360);

	szsinx = sinx * grTransform.scalez;
	szcosx = cosx * grTransform.scalez;
	sxsiny = siny * grTransform.scalex * (1/tan(grTransform.fovxangle*M_PI/360));
	sxcosy = cosy * grTransform.scalex * (1/tan(grTransform.fovxangle*M_PI/360));
	sysinx = sinx * grTransform.scaley * (1/tan(grTransform.fovyangle*M_PI/360));
	sycosx = cosx * grTransform.scaley * (1/tan(grTransform.fovyangle*M_PI/360));
}
